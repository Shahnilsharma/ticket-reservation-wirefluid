...
async function resetSeats() {
  ...
  const format = (rows: Array<{ status: 'AVAILABLE' | 'LOCKED' | 'SOLD'; _count: { _all: number } }>) =>
    rows
  ...
  console.log(`Before: ${format(before as Array<{ status: 'AVAILABLE' | 'LOCKED' | 'SOLD'; _count: { _all: number } }>)}.`);
  console.log(`After: ${format(after as Array<{ status: 'AVAILABLE' | 'LOCKED' | 'SOLD'; _count: { _all: number } }>)}.`);
}
