...
type SectionClientId = 'north' | 'north-east' | 'south-east' | 'south' | 'south-west' | 'north-west';

function toSectionClientId(sectionName: string): SectionClientId | null {
...
function toClientSeatStatus(status: 'AVAILABLE' | 'LOCKED' | 'SOLD') {
  return status.toLowerCase() as 'available' | 'locked' | 'sold';
}
...
async function releaseExpiredLocks(sectionId?: string) {
  const now = new Date();
  const where = {
...
export const getSections = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const groupedMap = new Map<string, { available: number; locked: number; sold: number }>();
    for (const item of grouped) {
      const current = groupedMap.get(item.sectionId) ?? { available: 0, locked: 0, sold: 0 };
      if (item.status === 'AVAILABLE') current.available = item._count._all;
      if (item.status === 'LOCKED') current.locked = item._count._all;
      if (item.status === 'SOLD') current.sold = item._count._all;
...
export const getSectionSeats = async (req: Request, res: Response, next: NextFunction) => {
 ...
 */
export const lockSeat = async (req: Request, res: Response, next: NextFunction) => {
 ...
 */
export const getTicketMetadata = async (req: Request, res: Response, next: NextFunction) => {
  const { id } = req.params;
 ...
 */
export const confirmPurchase = async (req: Request, res: Response, next: NextFunction) => {
 ...
 */
export const verifyEntry = async (req: Request, res: Response, next: NextFunction) => {
