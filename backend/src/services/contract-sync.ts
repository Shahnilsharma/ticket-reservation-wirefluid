...
const CONTRACT_ADDRESS = (process.env.WIREFLUID_CONTRACT_ADDRESS ?? '0x5D21dA3Fd25Af95f6b26E8b8784C431E06D5A940') as Address;
...
const sectionTierByName = new Map<string, 1 | 2 | 3>([
  ['north stand', 1],
  ['north-east stand', 1],
  ['south-east stand', 2],
  ['south stand', 2],
...
function getContractSectionTier(sectionName: string): 1 | 2 | 3 | null {
...
function shouldPreserveLock(seat: SeatRecord) {
  return seat.status === 'LOCKED' && seat.lockedUntil !== null && seat.lockedUntil > new Date();
}

async function syncSeatChunk(seats: SeatRecord[]) {
  ...
  const queryable = contracts.filter((item): item is typeof item & { tier: 1 | 2 | 3; args: [bigint, bigint, bigint, bigint] } => Boolean(item.args));
  ...
  const updates = queryable.flatMap((item, index) => {
    ...
    const desiredStatus: SeatRecord['status'] = onChainReserved
      ? 'SOLD'
    ...
    if (
      item.seat.status === desiredStatus &&
