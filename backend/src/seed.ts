...
type SectionSeedConfig = {
  id: string;
  name: string;
  basePrice: number;
  seatTarget: 1666 | 1667;
};
...
const SECTION_CONFIGS: SectionSeedConfig[] = [
...
function rowSeatCounts(target: number): number[] {
...
async function seed() {
  ...
  for (const section of SECTION_CONFIGS) {
    ...
    console.log(
      `Section "${section.name}" seeded with ${seats.length} seats (${counts.length - 1} full rows + ${counts[counts.length - 1]} seats in final row).`
    );
  }
